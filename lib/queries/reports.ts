...
type ClientPaymentRow = { assignment_id: string; payer_id: string | null; payment_date: string; amount_original: number | string; currency_original: string; amount_inr: number | string };
...
export async function getReportData(ownerId: string, range: ReportRange): Promise<ReportData> {
  ...
  const [assignmentResult, contactResult, roleResult, workerResult, clientPaymentResult, workerPaymentResult, expenseResult] = await Promise.all([
    ...
    supabase.from("client_payments").select("assignment_id, payer_id, payment_date, amount_original, currency_original, amount_inr").eq("owner_id", ownerId),
  ...
  const failed = [assignmentResult, contactResult, roleResult, workerResult, clientPaymentResult, workerPaymentResult, expenseResult].find((result) => result.error);
  ...
  const allClientPayments = (clientPaymentResult.data ?? []) as ClientPaymentRow[]; const rangedClientPayments = allClientPayments.filter((row) => inRange(row.payment_date, range));
  ...
  for (const assignment of assignments) { const matching = allClientPayments.filter((row) => row.assignment_id === assignment.id && row.currency_original === assignment.currency).reduce((sum, row) => sum + numberValue(row.amount_original), 0); unmatchedClientPayments += allClientPayments.filter((row) => row.assignment_id === assignment.id && row.currency_original !== assignment.currency).length; addCurrencyAmount(outstanding, assignment.currency, Math.max(numberValue(assignment.selling_price) - matching, 0)); }

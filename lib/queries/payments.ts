...
export async function getPaymentsData(
  ...
  const assignments = (assignmentResult.data ?? []) as PaymentAssignment[];
  ...
  let clientPayments: ClientPayment[] = (
    ...
    assignment: assignmentsById.get(payment.assignment_id) ?? null,
  ...
  if (filters.assignmentId) {
    clientPayments = clientPayments.filter(
      (payment) => payment.assignment_id === filters.assignmentId
    );
    workerPayments = workerPayments.filter(
      (payment) =>
